...
{
    static u8 u8Counter = 0;               // Tracks position within DAC.
    static u8 UserApp_au8sinTable[] =      // Sin signal with 256 samples (0 to 255)
    {
    ...
    if(u8Counter == 0xff)       // Basically max dac value in order to restart 
    {
        DAC1DATL = UserApp_au8sinTable[u8Counter]; //updates DAC and saves a value before rollover
        u8Counter = 0;
    }
    DAC1DATL = UserApp_au8sinTable[u8Counter]; // update DAC with new sin value
    u8Counter+=4;                           
} /* end UserAppRun */
